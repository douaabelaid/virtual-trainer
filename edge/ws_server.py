...
async def main() -> None:
    ...
    cs = os.getenv("CODESPACE_NAME", "localhost")
    
    logger.info("=" * 70 )
    logger.info("🚀 Virtual Trainer — Pose Detection Edge Server (Phase 2)")
    logger.info("=" * 70)
    logger.info(f"   Address         : ws://{WS_HOST}:{WS_PORT}")
    logger.info(f"   Target FPS      : {TARGET_FPS}")
    logger.info(f"   Max latency     : {MAX_LATENCY_MS}ms")
    logger.info(f"   MP complexity   : {MP_COMPLEXITY}")
    logger.info(f"   Detection conf  : {MP_DETECT_CONF}")
    logger.info(f"   Tracking conf   : {MP_TRACK_CONF}")
    logger.info("─" * 70)
    ...
    async with serve(
        ...
        logger.info("✅ Server ready — awaiting client connections …")
